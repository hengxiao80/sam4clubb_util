...
!
      ...
      do ifields=1,nfields
        ...
        print*, name,strlen1(long_name), long_name(1:strlen1(long_name)),'   ',c_max,c_min
        ...
        do n=0,nsubs-1
          ...
          do j=1+j0,ny+j0
            do i=1+i0,nx+i0
              length=length+1
              ! with UWM_MISC
              fld(i+nx_gl*(j-1)) = fmin+(byte(length)+32000)*max(fmax-fmin, 1.0e-20)/64000.
              ! without UWM_MISC
              ! fld(i+nx_gl*(j-1)) = fmin+(byte(length)+32000)*(fmax-fmin)/64000.
            end do
           end do
