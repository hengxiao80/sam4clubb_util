...
c variables:
	...
	integer filecount ! mwswong
        character(8) ncsuffix ! mwswong
	character(180) filename_com3D ! mwswong
...
c
	...
	print*,filename
        filename_com3D = filename  ! save for later
c---------------------------------------------------------------
...
98     continue
       ...
       write(6,*) 'filecount = ', filecount

c
c The output filename:

	do i=1,176
	  if(filename_com3D(i:i+5).eq.'.com3D') then
            write(ncsuffix,"(A,I1,A)") '_', filecount, '.nc   '   ! mwswong
	    !!!!mwswong filename(i:i+5)='_1.nc   '
            filename(i:i+5)=ncsuffix
	    EXIT
	  else if(i.eq.176) then
...
c
	...
	!do while(ifields.lt.nfields-2) !MWSWong: split between two parts
	!do while(ifields.lt.ceiling(nfields/2.)) !MWSWong: split between two parts

          if (ifields.gt.filecount*16-1) goto 98         
     ...
     &		   fmin+(byte(count)+32000.)*(fmax-fmin)/64000.
	  ...
	  print*,ifields,long_name,fldmax,fldmin
